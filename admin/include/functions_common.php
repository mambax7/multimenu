...
// Render data list
                       ...
                       $data['css'] ) = $xoopsDB -> fetchrow( $link_list ) )
	{
            ...
            if( $data['pid']==$old_main_pid || !$data['pid'] ) {
             ...
             if( $data['relative'] && $data['type']=='relative' ) {
                 ...
                 foreach( $data['relative'] as $relative ) { $is_displayed = strstr($_SERVER['REQUEST_URI'], trim($relative))?1:$is_displayed; }
                 }
 ...
 if( $is_image ) {
          ...
          $data['title']    = $is_title?multimenu_short_title( $data['title'], $text_max_length, '[...]' ):'';
                    ...
                    // Check sublink status
                     ...
                     if( $data['query'] ) {
                       ...
                       if( (!$data['pid'] && $data['link']) || ($data['pid']&& $data['link']) ) {
                         ...
                         $data['data_list'][++$backintime]['has_sub'] = $data['pid']?1:0; // Check wether the précédent mainlink has a sublink
                         $data['num_main']    = !$data['pid']?$main_i++:$main_i;
                         $data['num']         =  $i++;
                         $data['data_list'][] = $data; 
                         $status=1; $show = 1;
                       ...
                       foreach( $data_query['result'] as $data_q ) {
                         ...
                         $alt_title = $is_infobulle&&!$data_q['alt_title'] ? strip_tags( $data_q['title'] ) : strip_tags( $data_q['alt_title'] );
                         $data_q['alt_title'] = addslashes( substr($alt_title, 0, 256) ); // addslashes(ereg_replace('"', '', $alt_title));
                         ...
                         $data_q['num_main']   = $link_status=='link'?$main_i++:$main_i;
                         $data_q['num']        = $i++;
                         $data['data_list'][++$backintime]['has_sub'] = $data_q['pid']?1:0; // Check wether the précédent mainlink has a sublink
                         $data['data_list'][] = $data_q; // $xoopsTpl->append('data_list', $data_q);
                        ...
                        if( $show ) { $count_query_result = $count_query_result?$count_query_result:'0';
                                      $data['data_list'][$current_main_link]['title'] = $count_query_result?ereg_replace('#', '<font style="color:#E00;">'.$count_query_result.'</font>', $data['title']):ereg_replace('#', $count_query_result, $data['title']);
                                      $data['data_list'][$current_main_link]['alt_title'] = ereg_replace('#', $count_query_result, $data['alt_title']); }
             ...
             $data['data_list'][++$backintime]['has_sub'] = 0;
             unset($data['data_list'][1]);
...
Global $xoopsDB, $xoopsUser;
        ...
        $where  = multimenu_fixe_date( $where );

//        $where       = $where?ereg_replace($pattern, $replacement, $where):$where;

/*        $where  = $where?ereg_replace('{yesterday}',  date("M-d-Y",mktime(0, 0, 0, date("m"),  date("d")-1,date("Y")  )), $where):$where;    //date("M-d-Y",mktime(0, 0, 0, date("m"),date("d")-1,date("Y")))
        $where  = $where?ereg_replace('{last_week}',  date("M-d-Y",mktime(0, 0, 0, date("m"),  date("d")-7,date("Y")  )), $where):$where;
        $where  = $where?ereg_replace('{last_month}', date("M-d-Y",mktime(0, 0, 0, date("m")-1,date("d"),  date("Y")  )), $where):$where;
        $where  = $where?ereg_replace('{last_year}',  date("M-d-Y",mktime(0, 0, 0, date("m"),  date("d"),  date("Y")-1)), $where):'';
*/
...
//        $list_count = $xoopsDB->getRowsNum( $list );
 ...
 if( $image && !$description ) {
    	...
    	while(list( $id, $title, $description ) = $xoopsDB->fetchRow($list))
     	{
          $uri   = ereg_replace('{id}', $id, $url);
          // $title = !$title?$xoopsUser->getVar('uname'):$title;
          $datas[$i]['link']        = XOOPS_URL.'/'.$uri;
          $datas[$i]['title']       = $title;
...
function multimenu_short_title( $title='', $length=32, $tiddle='[...]' )
{
     ...
     $title = ereg_replace('{username}', XoopsUser::getUnameFromId($uid), $where_query)
