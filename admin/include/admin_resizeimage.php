...
function multimenu_thumb_resize ( $image_list, $xmax='200', $ymax='150', $thumb='thumb/', $image='', $fond_couleur='#FFFFFF', $force=0 ) {
/*
...
if( $force==1 ) {
  multimenu_delete_files( $thumb );
   } elseif( $force==2 ) {
     Global $extensions;
  multimenu_create_dir( $image );
  multimenu_move_files( $image, $image.'orig/' );
   }
          ...
          // si le fichier n'est pas une image :
              if(list($largeur,$hauteur,$extention) = @getimagesize(htmlentities($img))) {
                  ...
                  // creation de la miniature :
                      ...
                      list($fond_rouge,$fond_vert,$fond_bleu) = explode('-',multimenu_html2rgb($fond_couleur));
...
function multimenu_html2rgb($color)
{
    if ($color[0] == '#') {
...
function multimenu_delete_files( $dir='', $ext='html' ) {
...
function multimenu_move_files( $dir='', $newdir='', $extensions='jpg' ) {
// Global $extensions;
// Get image list
        //Using the opendir function
        $dir_handle = @opendir($dir);
...
$thePath = $directory;
	...
	if(@is_writable($thePath)){
		multimenu_chmod($thePath, $mode = 0777);
        Return $thePath;
	} elseif(!@is_dir($thePath)) {
    	        multimenu_mkdir($thePath);
        Return $thePath;
	} else {
        Return 0;
        }
...
function multimenu_mkdir($target)
{
	...
	if (multimenu_mkdir(substr($target,0,strrpos($target,'/')))) {
		if (!file_exists($target)) {
			$res = mkdir($target, 0777); // crawl back up & create dir tree
			multimenu_chmod($target);
			Global $xoopsModule;
...
function multimenu_chmod($target, $mode = 0777)
{
